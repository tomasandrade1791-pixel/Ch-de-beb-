...
(function(){
  ...
  function initEnvelopeAnimation(){
    const card=document.getElementById('envCard'); const closed=document.getElementById('envClosed'); const open=document.getElementById('envOpen'); const tap=document.getElementById('tapText');
  ...
  function initRulesIntroSplit(){
    ...
    const timer=setInterval(function(){
      ...
      clearInterval(timer);
      const baseRenderRule=window.renderRule; const baseNext=window.next; const basePrev=window.prev; let intro=true;
...
// A cena da Alice fica estática enquanto ajustamos o visual.
(function(){
  const style=document.createElement('style');
  style.textContent=`#alice .aliceArt{animation:none!important;opacity:1!important;transform:none!important;mix-blend-mode:normal!important;filter:none!important}#alice .aliceLight,#alice .aliceLight:before,#alice .aliceLight:after{display:none!important;animation:none!important}`;
  document.head.appendChild(style);
})();

// Usa diretamente a foto que foi recolocada no GitHub.
(function(){
  const aliceUrl='https://raw.githubusercontent.com/tomasandrade1791-pixel/Ch-de-beb-/main/IMG-20260919-WA0102.jpg?v=20260925';
  function forceAliceImage(){
    const img=document.querySelector('#alice .aliceArt');
    if(!img)return;
    if(!img.src.includes('IMG-20260919-WA0102.jpg?v=20260925')) img.src=aliceUrl;
    img.alt='Alice';
  ...
  function initAlice(){
    ...
    const section=document.getElementById('alice');
    if(section)new MutationObserver(forceAliceImage).observe(section,{attributes:true,attributeFilter:['class']});
  }
