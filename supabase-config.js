...
(function(){
  function restartAliceAnimation(){
    ...
    if(logo) logo.remove();
    if(old){ old.style.display='block'; old.style.animation='none'; old.style.opacity='1'; old.style.transform='none'; }
...
(function(){
  const aliceUrl='https://raw.githubusercontent.com/tomasandrade1791-pixel/Ch-de-beb-/main/alice.svg?v=20260925-clean';
  function applyAliceImage(){
    const img=document.querySelector('#alice .aliceArt');
    if(!img) return;
    if(img.src!==aliceUrl) img.src=aliceUrl;
    img.alt='Alice'; img.style.display='block'; img.style.animation='none'; img.style.opacity='1'; img.style.transform='none';
  }
  function start(){
    applyAliceImage(); setTimeout(applyAliceImage,50); setTimeout(applyAliceImage,300);
    const section=document.getElementById('alice');
    if(section){new MutationObserver(function(){if(section.classList.contains('on')) applyAliceImage();}).observe(section,{attributes:true,attributeFilter:['class']});}
  }
...
(function(){
  const style=document.createElement('style');
  style.textContent=`
    #alice .aliceArt{animation:none!important;opacity:1!important;transform:none!important;mix-blend-mode:normal!important;filter:none!important;clip-path:inset(0 0 36% 0)!important;}
